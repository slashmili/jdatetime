...
class TestJDateTime(TestCase):
    ...
    def test_date_conversion_integer_input(self):
        ...
        self.assertEqual(
            True,
            jd_datetime
            == jdatetime.datetime.combine(d_check_with, jdatetime.time(14, 15, 16)),
        )
    ...
    def test_strptime_invalid_date_string_b_directive(self):
        with self.assertRaises(
            ValueError, msg="time data '14 DRO 1400' does not match format '%d %b %Y'"
        ):
            jdatetime.datetime.strptime("14 DRO 1400", "%d %b %Y")

    def test_strptime_invalid_date_string_B_directive(self):
        with self.assertRaises(
            ValueError, msg="time data '14 ordi 1400' does not match format '%d %B %Y'"
        ):
    ...
    def test_strptime_handle_z_directive(self):
        tests = [
            ...
            (
                "+012345.012345",
                "%z",
                datetime.timedelta(seconds=5025, microseconds=12345),
            ),
            (
                "-012345.012345",
                "%z",
            ...
            (
                "+01:23:45.123",
                "%z",
                datetime.timedelta(seconds=5025, microseconds=123000),
            ),
        ]
    ...
    def test_strptime_invalid_date_string_z_directive(self):
        tests = [
            ...
            (
                "+012345123456",
                "%z",
                "time data '+012345123456' does not match format '%z'",
            ),
        ]
...
class TestJdatetimeComparison(TestCase):
    ...
    def test_eq_datetime_diff_tz(self):
        ...
        self.assertEqual(
            dt_teh, dt_gmt, "In standrd python datetime, __eq__ considers timezone"
        )
...
class TestJdatetimeGetSetLocale(TestCase):
    ...
    @skipUnless(
        greenlet_installed, "greenelts ident is used when greenlet module is installed"
    )
    ...
    def test_set_locale_sets_default_locale_for_date_objects(self):
        ...
        fa_record = []
        fa_th = threading.Thread(
            target=record_locale_formatted_date, args=(fa_record, jdatetime.FA_LOCALE)
        )
        fa_th.start()
        fa_th.join()
    ...
    @skipUnless(
        greenlet_installed, "greenlets ident is used when greenlet module is installed"
    )
    ...
    def test_unknown_type_operations(self):
        ...
        with self.assertRaisesRegex(
            TypeError, r"unsupported operand type\(s\) for \-=: 'datetime' and 'object'"
        ):
            dt -= unknown_type
