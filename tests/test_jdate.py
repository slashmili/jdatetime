...
class TestJDate(TestCase):
    ...
    def test_unknown_type_operations(self):
        ...
        with self.assertRaisesRegex(
            TypeError, r"unsupported operand type\(s\) for \+=: 'date' and 'object'"
        ):
            date += unknown_type
    ...
    def test_max_year(self):
        ...
        with self.assertRaises(
            ValueError, msg="Should raise an exception when we go over date.max"
        ):
    ...
    def test_min_year(self):
        ...
        with self.assertRaises(
            ValueError, msg="Should raise an exception when we ge below date.min"
        ):
